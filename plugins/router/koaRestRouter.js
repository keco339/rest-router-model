...
module.exports = function restRouter(resourceConfig, extendBusinesses, dbConfig,options) {
    ...
    const seneca = Seneca({tag: options.serverName,timeout: 60*1000,log:'silent',debug:{undead: true},fixedargs:{fatal:false}});
    ...
    // 资源业务插件
    seneca.use('../../plugins/business/business',{resourceConfig,dbConfig:dbConfig,extendBusinesses:extendBusinesses});

    // HTTP URL --> Pattern
    let koa_router = options.router || KoaRouter();
    seneca.use(SenecaWeb, {context: koa_router, adapter: seneca_web_adapter, routes: generateRouteMaps(resourceConfig)});
    seneca.ready(() => {
